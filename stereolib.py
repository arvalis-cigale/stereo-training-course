...
# Computes the height estimation from a height distribution, 
def height_estimation(height_array, method, percentile=None):

    # Initialization of return values
    ret = 0
    height = 0

    # Calcul de l'histogramme des pixels au-dessus de la valeur
    # du sol estimée, avec un échantillonnage suffisant
    # pour la résolution souhaitée sur la hauteur ( < cm)
    height_hist, depth_bins = np.histogram(height_array, bins=1024)
    # Filtrage assez sévère pour éliminer les pixels disseminés
    # du fait du bruit, et ne faire ressortir que les étages
    # représentatifs
    height_hist = gaussian_filter1d(height_hist, sigma=10)

    grad_depth_hist = np.gradient(height_hist)

    # Passage à la valeur absolue du gradient
    grad_depth_hist = abs(grad_depth_hist)
    # Normalisation
    grad_depth_hist = grad_depth_hist / np.max(grad_depth_hist)

    # Histogramme cumulé
    cum_depth_hist = np.cumsum(height_hist)

    if method == "percentile":
        # Recherche de l'abscisse à laquelle on passe au-dessus
        # de la valeur de percentile souhaitée (c'est le premier
        # indice du tableau de retour)
        dist_threshold = (percentile / 100) * np.max(cum_depth_hist)
        distance_index = np.argwhere(cum_depth_hist > dist_threshold)

        if len(distance_index) > 0:
            # On interpole linéairement pour avoir la valeur correspondant
            # à percentile
            if distance_index[0][0] > 0:

                distance_index = distance_index[0][0] - 1

                a = depth_bins[distance_index]
                b = depth_bins[distance_index + 1]
                ya = cum_depth_hist[distance_index]
                yb = cum_depth_hist[distance_index + 1]

                height = (b - a) * (dist_threshold - ya) / (yb - ya) + a
            else:
                ret = -1
        else:
            ret = -2
        ...
        height = np.mean(height_array)
    elif method == "median":
        height = np.median(height_array)
