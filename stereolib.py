...
def load_calibration_from_numpy(calibration_path):
    ...
    # We load rotation and translation matrices resulting of the stereo pair calibration
    # Rotation matrix
...
def load_calibration_from_yaml(calibration_filename):
    ...
    # We read the calibration file content
    s = cv.FileStorage()
    ...
    # Stereo pair formed by left and right cameras
    # We load rotation and translation matrices resulting of the stereo pair calibration
...
# Function that reads an image given its path
...
# Function that writes an image on the disk
...
# Function that displays a list of images side-by-side horizontally
def display_images(images_list):
    
    # We create a subdivision to display images side-by-side on one row
...
# Function that rectifies an image from the maps generated by OpenCV 'stereoRectify'
# See './documentation/OpenCV - stereoRectify.pdf' and './documentation/OpenCV - remap.pdf'
...
# Function that normalizes an image as a 8 bits values image (values in the [0;255] interval)
def normalize_image(img):
    # Min and max bounds of the input image
    min_src = int(np.min(img))
    max_src = int(np.round(np.max(img)))

    # Max value for 8 bits output : 256-1 = 255
    max_dst = 2**8 - 1
    
    # We make a copy of the input image, to ensure it won't be affected
    img_norm = np.copy(img)
    # We normalize the copied image from [min_src;max_src] to [0;1] 
    img_norm = (img_norm - min_src) / (max_src - min_src)
    # We multiply by max_dst (= 255) so that output values are in the [0;255] interval
    img_norm = max_dst * img_norm

    # We ensure that there are no rounded values outside the wanted interval
    img_norm[img_norm < 0] = 0
    img_norm[img_norm > max_dst] = max_dst

    # We format the output as a 8 bits unsigned integer image
    img_norm_int = np.copy(np.round(img_norm)).astype(np.uint8)
        
    return img_norm_int

# Function that writes a point cloud with RGB information into the laz format (compressed 'las')
def write_point_cloud(file_name, xyz_image, bgr_image):
    ...
    # We finally write the file on the disk
    ptCloud.write(file_name)

# Function that converts depth value (in mm) to disparity (in pixels), given the focal length (in pixels) 
# and the baseline (in mm) of the stereo pair
...
# Function that converts disparity (in pixels) value to depth (in mm), given the focal length  (in pixels)
# and the baseline of the stereo pair (in mm)
...
# Function that computes the number of disparities needed by the SGBM algorithm from min and max desired values.
# The min_disparity value will be kept, but the max_disparity value will be potentially increased, as the number 
# of expected disparities must be a multiple of 16 (as SGBM was implemented in OpenCV, read from the doc). 
...
# Function that computes the correspondence matching between each pixel of the left and right image of a stereo camera pair,
...
def stereo_processing(left_img,
    ...
    # We create a matcher object with appropriate parameters 
    ...
    # If the 'wls_activation' flag is active, we create a WLS filter object 
    if wls_activation:
    ...
    # We now launch the SGBM matching algorithm
    try:
        raw_disparity = left_matcher.compute(
            np.copy(left_img), np.copy(right_img)
        ).astype(np.float32)

        # We rescale disparity output, as explained in the OpenCV documentation
    ...
    # If the 'wls_activation' flag is active
    if wls_activation:
        
        # We compute disparity from right to left image
        disparity_right = np.zeros(right_img.shape)
        
        try:
            # We create a matcher object for the right image
            right_matcher = cv.ximgproc.createRightMatcher(left_matcher)

            # We launch the SGBM matching algorithm from the right image
            ...
            # We compute the WLS filtering comparing left-to-right and right-to-left 
            # obtained disparity maps (the following function needs dispartity maps 
            # before rescaling)
            ...
            # We launch the generation of the confidence map to get metrics about
            # matching and filtering quality
...
# Function that computes the normalized total gradient between the left image, 
# and the right-on-left mapped image obtained by applying disparity to each pixel
# Arguments :
...
def normalized_total_gradient(
    ...
    # We fill the right_on_left_img pixels, taking pixels in the right image at the same line 
        ...
        for j_left in range(disparity_left.shape[1]):
            ...
            if j_right >= 0:
                ...
                if (int(disparity_left[i_left, j_left])) >= min_disparity:
                    # If so, we fill the right-on-left image pixel at [i_left, j_left]
                    # with the pixel in the right image at [i_right, j_right]
                    right_on_left_img[i_left, j_left] = gray_img_right[i_right, j_right]
                else:
                    # We fill both left and right-on-left pixel with extem values, 
                    # so that the resulting gradient is maximal (i.e. not good)
